...
(function() {
  var BLOCKS, BLOCK_ELEMENTS, BRACKETS, ELEMENT_NODE, PARENS, PUNCTUATION, QPB, QUOTES, SHORTENABLE_WORDS, TEXTUAL_ELEMENTS, TEXT_NODE, WORD_REGEX, key,
  ...
  String.prototype.getStatements = function() {
    var char, chars, closing, current, currentLast, dontBreak, isCloseToCap, isContinuation, lastCap, lastCapDelta, statements;
    chars = [];
    current = [];
    statements = [];
    closing = ".";
    ...
    while (chars.length > 0) {
      ...
      if ((__indexOf.call(QUOTES, char) >= 0 && __indexOf.call(QUOTES, closing) >= 0) || (__indexOf.call(PARENS, char) >= 0 && __indexOf.call(PARENS, closing) >= 0) || (__indexOf.call(BRACKETS, char) >= 0 && __indexOf.call(BRACKETS, closing) >= 0) || (__indexOf.call(PUNCTUATION, char) >= 0 && __indexOf.call(PUNCTUATION, closing) >= 0) || (chars.length === 0)) {
        ...
        dontBreak = !(isContinuation || isCloseToCap);
        if (chars.length === 0 || dontBreak) {
          if (current.length > 0) {
      ...
      } else if ((__indexOf.call(QPB, char) >= 0)) {
        closing = char;
      }
    }
    return statements;
